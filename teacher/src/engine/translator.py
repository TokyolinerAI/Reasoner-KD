...
from model.mainmodel import Reasoner
from dataloader import VARDataset
from utils import get_rank, is_distributed, reduce_tensor, get_world_size
...
class Translator(object):
    ...
    def translate_batch_greedy(self,
        ...
        with torch.no_grad():
            video_embeddings = rt_model.video_encoder(video_features=video_features, video_mask=video_mask,
            ...
            for k in range(1, self.opt.K):
                ...
                prev_sentence_embeddings = torch.stack(prev_sentence_embeddings, dim=0).contiguous().view(-1,
                                                                                                          *_sentence_embedding.shape[
                                                                                                           1:])
                prev_confidence_vector = torch.stack(prev_confidence_vector, dim=0).contiguous().view(-1,
                                                                                                      *_confidence_vector.shape[
                                                                                                       1:])
