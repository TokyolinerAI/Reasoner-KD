'''
### **3. `teacher/src/runner.py` (修改后)**
*   **改动**：在`get_args()`中添加`--model_mode`参数，并在`main()`中传递给`VARDataset`。
'''
'''
...
def get_args():
    ...
    parser.add_argument('--model_name', type=str, default='Reasoner')
    ...
    # <<<<<<<<<<<<<<< 核心修改点 >>>>>>>>>>>>>>>
    parser.add_argument('--model_mode', type=str, default='teacher', choices=['student', 'teacher'],
                        help='Specify whether to run in teacher or student mode. Teacher mode disables hypothesis masking.')
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    parser.add_argument('--trans_sta_epoch', type=int, default=3)

    # * model overall config
    parser.add_argument('--hidden_size', type=int, default=768)
    parser.add_argument('--intermediate_size', type=int, default=1536) #teacher。学生768*2
    parser.add_argument('--vocab_size', type=int, help='number of words in the vocabulary')
    parser.add_argument('--word_vec_size', type=int, default=300)
    parser.add_argument('--video_feature_size', type=int, default=3072, help='2048 appearance + 1024 flow') #teacher 学生2048
    parser.add_argument('--max_v_len', type=int, default=50,
    ...
    # --- 核心修改点：智能判断 local_rank ---
    # 如果 LOCAL_RANK 环境变量存在 (由 torchrun/launch 设置)，则使用它
    # 如果不存在，则默认为 0，表示这是单进程/主进程
...
def main():
    ...
    opt.glove_path = os.path.abspath(opt.glove_path)  # <-- 建议添加这一行，让路径更健壮
    # --- END: 添加结束 ---

    init_seed(opt.seed, cuda_deterministic=True)

    # --- 核心修改点：智能判断是否为分布式模式 ---
    # 只有当 LOCAL_RANK 环境变量存在时，才认为是分布式模式
    ...
    train_dataset = VARDataset(
        ...
        model_mode=opt.model_mode # <<<<<<<<<<<<<<< 核心修改点 >>>>>>>>>>>>>>>
        )
    ...
    train_loader = DataLoader(train_dataset, collate_fn=sentences_collate,
                              ...
                              shuffle=(train_sampler is None), # 单卡时 shuffle=True
    ...
    if opt.glove_path is not None:
        if hasattr(model, 'embeddings_decs'):
            ...
            for k in range(opt.K):
                glove_embeddings = torch.load(
                    os.path.join(opt.glove_path, opt.glove_version),
                    weights_only=False
                )
                model.embeddings_decs[k].txt_emb.set_pretrained_embedding(
                    torch.from_numpy(glove_embeddings).float(), freeze=opt.freeze_glove)
    ...
    else: model = model.to(device)  # 直接将模型移动到设备
