...
class VARDataset(Dataset):
    ...
    def __init__(self, dset_name, data_dir,
                 ...
                 word2idx_path=None, model_mode='student'):  # 增加 model_mode 参数
        self.dset_name = dset_name
        self.data_dir = data_dir  # containing training data
        self.mode = mode
        self.model_mode = model_mode  # 新增: 'student' or 'teacher'
        self.sample_mode = sample_mode
        self.K = K  # * number of cascade decoders
        assert K >= 1

        if self.model_mode == 'teacher':
            logging.info("Dataset initialized in TEACHER mode. Hypothesis masking is DISABLED.")
        else:
            logging.info("Dataset initialized in STUDENT mode. Hypothesis masking is ENABLED.")
        ...
        if (word2idx_path is None) or (not os.path.exists(word2idx_path)):
            ...
            word2idx_path = os.path.join(data_dir, 'vocab_feature', 'min3_word2idx_6B.json')
    ...
    def fix_missing(self):
        ...
        if not self.data:
            print("ERROR: self.data is empty BEFORE fix_missing() is called. Check _load_data method.")
        else:
            first_video_name = self.data[0]['video_ids'][0]
            test_path_resnet = os.path.join(self.video_feature_dir, f"{first_video_name}_resnet.npy")

            print(f"Data Dir received by VARDataset: {self.data_dir}")
            print(f"Constructed video_feature_dir: {self.video_feature_dir}")
            print(f"Full path being checked: {test_path_resnet}")
            print(f"Does this path exist? {os.path.exists(test_path_resnet)}")

        import sys
        sys.exit("--- DEBUGGING COMPLETE ---")  # 打印完后直接退出程序
        # --- END: 添加调试代码 ---
        '''
        missing_video_names = []
        missing_examples = []
    ...
    def convert_example_to_features(self, example):
        ...
        # * visual feature for aux loss (always unmasked, so we prepare it before potential masking)
        _, f_video_all_mask, f_feat, f_video_temporal_tokens = self._construct_entire_video_features(
            single_video_features)

        # <<<<<<<<<<<<<<<<<<<< 核心修改点 >>>>>>>>>>>>>>>>>>>>
        # In teacher mode, DO NOT mask the visual input.
        if self.model_mode == 'student':
        ...
        data = dict(
            ...
            # The 'unmasked' input for student is the full video, for teacher it's the same as 'encoder_input'
