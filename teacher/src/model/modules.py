...
import numpy as np
import math
#from teacher.src.utils import get_rank
...
class RelativeTemEmbedding(nn.Module):
    ...
    def _meta_forward(self, rp_bucket, query_emb):
        ...
        if query_emb is None and self.relative_mode == 'bias':
            ...
            B = query_emb.shape[0]
            if self.on_value:
                weight = torch.index_select(self.lookup_table_weight, 1, rp_bucket.flatten()).view(1, L, L,
                                                                                                   self.head_dim)
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, query_states, key_states, value_states, attention_mask, temporal_tokens=None):
        ...
        if self.rpe_q is not None:
            attention_scores += self.rpe_q(temporal_tokens, key_layer / math.sqrt(self.attention_head_size)).transpose(
                2, 3)
...
class BertSelfAttentionSen(nn.Module):
    ...
    def forward(self, query_states, key_states, value_states, attention_mask, confidence_vector=None):
        ...
        if self.rpe_q is not None:
            attention_scores += self.rpe_q(confidence_vector,
                                           key_layer / math.sqrt(self.attention_head_size)).transpose(2, 3)
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, temporal_tokens=None, confidence_vector=None):
        ...
        """
        inputs = {'query_states': input_tensor, 'key_states': input_tensor, 'value_states': input_tensor,
