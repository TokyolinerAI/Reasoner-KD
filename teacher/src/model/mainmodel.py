...
class Reasoner(nn.Module):
    ...
    def _forward_refactored(self,
        ...
        aux_encoder_loss = F.mse_loss(_video_embeddings,
                                      _unmasked_video_embeddings,
                                      reduction='sum')
        aux_encoder_loss = aux_encoder_loss / (torch.sum(unmasked_encoder_input['video_mask']).detach() + 1e-8)
        '''
        # 打印视频编码器辅助损失
        print("-" * 60)
        print("[TEACHER LOSS DIAGNOSIS]")
        print(f"  - Video Encoder Aux Loss: {aux_encoder_loss.item():.6f}")
        '''
        ...
        # --- 插入点 2: 收集 Pass 1 (k=0) 的 Logits ---
        if mode == 'generate_soft_labels':
            # k=0 的分数，形状: (B*S, L, V)
            all_teacher_logits.append(prediction_scores_dec1.detach())
        # --- 插入点 2 结束 ---
        ...
        main_caption_loss = torch.tensor(0.0, device=video_embeddings.device)

        # 3. 中间解码器循环 (k=1 到 k=K-1)
        for k in range(1, self.config.K):  # 循环 K-1 次

            # (A) 计算上一步 (k-1) 的辅助损失
            k_minus_1_scores = prev_prediction_scores
            current_hard_labels = input_labels

            loss_k_minus_1 = self.loss_func(
                k_minus_1_scores.view(-1, self.config.vocab_size),
                current_hard_labels.view(-1)
            )
            total_aux_caption_loss += self.config.loss_aux_caption * loss_k_minus_1

            # (B) 准备并运行第 k 步解码器
            # (省略 _conf_n_sen_emb_construction，假设它在您的文件版本中存在)
            sentence_embeddings, confidence_vector = self._conf_n_sen_emb_construction(metas,
                                                                                       prev_prediction_scores,
                                                                                       prev_outputs, input_masks,
                                                                                       n_S=S)
            input_labels, input_ids, token_type_ids, input_masks = \
                _concat_list_of_tensors(
            ...
            # --- 插入点 3: 收集循环中 (k > 0) 的 Logits ---
            if mode == 'generate_soft_labels':
                # k > 0 的分数，形状: (B*S, L, V)
                all_teacher_logits.append(current_prediction_scores.detach())
                # --- 插入点 3 结束 ---
        ...
        # 4. 计算最终 (K-1) 步的主损失
        # (此时, prev_prediction_scores 保存的是第 K-1 解码器的输出)
        final_scores = prev_prediction_scores
        final_hard_labels = input_labels  # (input_labels 在循环最后一次迭代时被正确设置)

        main_caption_loss = self.config.loss_main_caption * self.loss_func(
            final_scores.view(-1, self.config.vocab_size),
            final_hard_labels.view(-1)
        )
        # 4. 最终返回
        # --- 核心修改点 4: 返回 K 个 Logits 或 训练损失 ---
        if mode == 'generate_soft_labels':
            if not all_teacher_logits:
                # 处理 K=0 或 K=1 但逻辑出错的情况
                return None
                # 堆叠 K 个 Logits 张量
            # 最终形状: (K, B*S, L, V)
            return all_teacher_logits # <-- 直接返回列表
        else:
            # 训练模式：返回总损失
            caption_loss = (self.config.loss_aux_weight * aux_encoder_loss +
                            total_aux_caption_loss +
                            main_caption_loss)

            # 保存最终预测用于计算准确率
            self.last_pred_scores = prev_prediction_scores.detach()
            self.last_gold_labels = input_labels.detach()
