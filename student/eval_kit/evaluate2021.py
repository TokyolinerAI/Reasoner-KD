...
class VAREvaluator(object):
    ...
    def example_evaluate(self, prediction):
        ...
        try:
            ...
            if self.verbose: print('computing %s score...' % (method))

            kargs = {'gts':gts, 'res':res}
            score, scores = scorer.compute_score(**kargs) 

            output[method] = score
            if self.verbose: print("Calculated %s: %0.3f" % (method, score))
        except Exception as e:
            print(f"警告：计算 BertScore 时出错: {e}。将分数设置为0。")
            output["BertScore"] = 0.0
