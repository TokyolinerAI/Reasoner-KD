...
from dataloader import prepare_batch_inputs
from .translator import Translator
from utils import save_json, load_json, get_rank, is_distributed, get_world_size, dist_log
...
def translate_w_metrics(checkpoint, eval_data_loader, opt, model=None, eval_mode='test'):
    ...
    with torch.no_grad():
        ...
        res_filepath = os.path.abspath(
            opt.save_model + '_tmp_greedy_pred_{}_{}.json'.format(eval_mode, int(get_rank())))
    ...
    if opt.local_rank <= 0:
        ...
        file_list = [os.path.abspath(opt.save_model + '_tmp_greedy_pred_{}_{}.json'.format(eval_mode, rank)) for rank in
